...
def setup_mdl_args(args, model_path):
    ...
    :param args: dict with all the argument values
    :param model_path: specify what model path you are loading args for
    :return model_args: dict with all the argument values from the loaded model
    :return model_path: updated model path (in case it needed to be downloaded from gcs)
    '''
    #if the model_path was not specified 
    if model_path is None:
        ...
        model_path = gcs_model_exists(model_path, args.bucket_name, args.exp_dir, args.bucket)
        
        #(2): load the args used 
        model_args = load_args(args, model_path) #if no args.pkl, will return current args, else will load args

        #(3): there are two potential file types to be setting up arguments for
    
        # if loading a finetuned model, that is args.finetuned_mdl_path = orig_path, make sure the pretrained model is available on your local machine 
        if args.finetuned_mdl_path == orig_path:
            if model_args.bucket_name != args.bucket_name: #if the bucket is not the same as the current bucket, initialize the bucket for downloading
                ...
                bucket = args.bucket
            model_args.pretrained_mdl_path = gcs_model_exists(model_args.pretrained_mdl_path, model_args.bucket_name, args.exp_dir, bucket)
