...
def train(args, audio_model, train_loader, val_loader=None):
    ...
    #(2) set GPU if available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print('running on ' + str(device))
    torch.set_grad_enabled(True)

    #(3) Initialize all of the statistics we want to keep track of
    ...
    while epoch < args.epochs + 1:
        ...
        for i, batch in enumerate(train_loader):
            #each batch comes out with a sample containing the waveform, fbank, targets, sample rate
            audio_input = batch['fbank'] #get fbank as audio input [batch size, target length, n mel bins]
            B = audio_input.shape[0]
            audio_input = audio_input.to(device, non_blocking=True) #send to device
            labels = batch['targets']
            labels = labels.to(device, non_blocking=True) #send to device
            ...
            if global_step <= 1000 and global_step % 50 == 0 and args.warmup == True:
                for group_id, param_group in enumerate(optimizer.param_groups):
                    ...
                    print('warm-up learning rate is {:f}'.format(param_group['lr'])) #TODO: printing that 
        ...
        #(10) optional validation + validation metrics
        if validation:
    ...
    if args.wa:
        audio_model, stats = get_wa(audio_model, val_loader, args, args.wa_start, args.wa_end)
        if validation:
            assert stats is not None, 'error in validation with weighted average - did not return stats'
            mAP = np.mean([stat['AP'] for stat in stats])
            mAUC = np.mean([stat['auc'] for stat in stats])
            middle_ps = [stat['precisions'][int(len(stat['precisions'])/2)] for stat in stats]
            middle_rs = [stat['recalls'][int(len(stat['recalls'])/2)] for stat in stats]
...
def get_wa(audio_model, val_loader, args, start_epoch, end_epoch):
    if val_loader is None:
        validation = False
    else:
        validation = True
    ...
    if validation:
        ...
        return audio_model, stats
    else:
        return audio_model, None
    

def evaluation(args,audio_model, eval_loader, val_loader):
    '''
    Run evaluation loop - if you want to change metrics, look at validate function
    '''
    if val_loader is None:
        validation = False
    else:
